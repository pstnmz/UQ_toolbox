...
# CLI args
parser = argparse.ArgumentParser(description="Train Vision Transformer (ViT-B/16) on medMNIST dataset")
...
parser.add_argument("--dropout_rate", type=float, default=0.1, help="Dropout rate (default: 0.1 for ViT)")
parser.add_argument("--learning_rate", type=float, default=0.0001, help="Learning rate (default: 0.0001 for ViT)")
...
def shutdown_dataloader_workers_and_clear_MONAI_cache(dl):
    ...
    try:
        ...
        print("Warning: failed to clear MONAI cache for fold")
        pass
    ...
    try:
        ...
        print("Warning: failed to empty CUDA cache for fold")
        pass
...
for flag, color, batch_size, use_randaugment in zip(flags, colors, batch_sizes, use_randaugments):
    print(f"Training ViT on {flag} with color={color}, batch_size={batch_size}, use_dropout={use_dropout_arg}, dropout_rate={dropout_rate_arg}, lr={learning_rate_arg}")
    ...
    exp_dir = os.path.join(runs_dir, flag, f"vit_b_16_{size}_{timestamp}_randaug{int(use_randaugment)}_numepochs{num_epochs}_bs{batch_size}{dropout_suffix}")
    ...
    for fold_idx, train_loader, val_loader in fold_gen:
        ...
        model_path = os.path.join(exp_dir, f'vit_b_16_{flag}_224_{fold_idx}.pt')
