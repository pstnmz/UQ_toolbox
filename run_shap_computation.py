...
if __name__ == '__main__':
    ...
    processes = []
    for fold in range(3):
        p = mp.Process(target=compute_shap_for_fold, args=(fold, models[fold], test_loader, devices[fold], results))
        p.start()
        processes.append(p)
    ...
    for fold in range(3, 5):
        compute_shap_for_fold(fold, models[fold], test_loader, devices[fold], results)
    ...
    with open('/mnt/data/psteinmetz/archive_notebooks/Documents/medMNIST/shap_results.pkl', 'wb') as f:
        pickle.dump({
