...
@delayed
def process_slice(case, slice_idx, final_mask, final_image, features_40_slice, features_40_slice_resized):
    """
    Process a single slice of the mask and compute the metrics.
    """
    results = {}
    slice_ma_for_db = final_mask[slice_idx, :, :]
    ...
    if size_slice > 0.1 * biggest_cut_size:
        ...
        # PCA
        results, features_eigenvect, a, b, ellipticity = for_ellipse_creation(
            sitk_mask, features_40_slice, image, method='PCA'
        )
        ...
        res_MCD, di_MCD, _, mask_ell_MCD, center_MCD = compute_metrics_ellipse(
        ...
        res_rpca, di_rpca, _, mask_ell_rpca, center_rpca = (
            results_rpca[key] for key in ['residuals', 'dice', 'xy', 'mask_ellipse', 'center']
        )
        
        # PCA Metrics
        res, di, _, mask_ell, center = compute_metrics_ellipse(a, b, features_eigenvect, sitk_mask)
        res_resized, di_resized, _, _, _ = compute_metrics_ellipse(
            a_resized, b_resized, features_eigenvect_resized, resized_mask
        )
        
        print(f"Case {case}, Slice {slice_idx} - Dice PCA: {di:.4f}, MCD: {di_MCD:.4f}, RPCA: {di_rpca:.4f}")
    ...
    return {f"{case}_{slice_idx}": results}


def process_case(case):
    """
    Process an entire case and distribute the work across slices.
    """
    results_case = {}
    ...
    thresh = cases_thresh.get(f"thresh_{case.split('/')[-1]}", 0.2)
    final_image, final_mask, _ = apply_thresh(a_im, a_ma, a_ma_bb, thresh)
    
    # Identify slices to process
    delayed_tasks = []
    biggest_cut_size = np.max([np.count_nonzero(final_mask[k, :, :]) for k in range(final_mask.shape[0])])
    for k in range(final_mask.shape[0]):
        size_slice = np.count_nonzero(final_mask[k, :, :])
        if size_slice > 0.1 * biggest_cut_size:
            delayed_tasks.append(
                process_slice(case, k, final_mask, final_image, None, None)
            )
    
    # Compute results for all slices
    results_case = compute(*delayed_tasks)
    return {k: v for result in results_case for k, v in result.items()}


# Main workflow
path_cases = glob.glob('/mnt/data/psteinmetz/neotex/to_process/Baseline_data/*')
all_cases_tasks = [delayed(process_case)(case) for case in path_cases]
all_results = compute(*all_cases_tasks)

# Combine and save results
final_results = {k: v for case_result in all_results for k, v in case_result.items()}
with open('/mnt/data/psteinmetz/archive_notebooks/Documents/baseline_results_dask.pickle', 'wb') as handle:
    pkl.dump(final_results, handle, protocol=pkl.HIGHEST_PROTOCOL)